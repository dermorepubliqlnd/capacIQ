-- Phase 12 (2026-08-21): Manual becomes a real 3rd scheduling Mode (not a
-- conditional override folded into Capacity-Based, see WbsPlanning.tsx).
-- Widen every mode-related CHECK constraint to allow 'manual', and update
-- decide_baseline_request so Manual-mode baselines/re-baselines read dates
-- from the same "_standard" JSON slot Capacity-Based used to own --
-- WbsPlanning.tsx's buildTaskSnapshotPayload now populates that slot with
-- Manual's own dates whenever activeMode = 'manual'. No new columns.
-- Applied live 2026-08-21 via Supabase SQL editor -- see this file for the
-- exact statements run.

alter table project_plan_versions drop constraint if exists project_plan_versions_mode_check;
alter table project_plan_versions add constraint project_plan_versions_mode_check check (mode in ('full_capacity','standard','manual'));

alter table project_baselines drop constraint if exists project_baselines_mode_check;
alter table project_baselines add constraint project_baselines_mode_check check (mode in ('full_capacity','standard','manual'));

alter table project_closeouts drop constraint if exists project_closeouts_mode_check;
alter table project_closeouts add constraint project_closeouts_mode_check check (mode in ('full_capacity','standard','manual'));

alter table task_planning_snapshots drop constraint if exists task_planning_snapshots_mode_check;
alter table task_planning_snapshots add constraint task_planning_snapshots_mode_check check (mode in ('full_capacity','standard','capacity_based','manual'));

-- decide_baseline_request: identical to phase15_migration.sql's version,
-- with two changes: (1) the draft-branch mode guard now allows 'manual',
-- (2) both "which dates to roll up" CASE expressions switched from
-- `= 'standard'` to `<> 'full_capacity'` so 'manual' also reads from the
-- start_date_standard/end_date_standard JSON fields (the slot Manual's
-- dates now live in).
